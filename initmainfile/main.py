...
def true_false_defined_use(variable):
    #エラー確認 
    error_detection(variable)
    with open("json/true_false_list.json", "r", encoding="utf-8") as file:
        true_false_list = json.load(file)
    test_variable = true_false_list[variable]
    print(test_variable)

def error_detection(variable):
    with open("json/true_false_list.json", "r", encoding="utf-8") as file:
        true_false_list = json.load(file)
    if variable in true_false_list.keys():
        pass
    else:
        print("error: Variables " + variable + " is not difined")
        exit()

true_false_defined_use("true")
true_false_defined_use("fewjfiewjofewi")
